...
contract CaseNft is ERC1155, Ownable {

    struct Case {
        string form_factor;
        string airflow_grade;
        string grade;
    }

    uint256 public constant Case = 0;

    mapping (uint256 => Case) private _tokendetails;

        constructor(string memory name, string memory symbol) public ERC1155(name, symbol) {
        _mint(msg.sender, Case, 1, "");
    }
